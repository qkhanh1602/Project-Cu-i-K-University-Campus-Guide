...
def minimax(stage: Stage) -> SearchResult:
    ...
    recorder = TraceRecorder("Minimax", limit=700)
    ...
    while frontier and expanded < 700:
        ...
        if current == goal:
            ...
            recorder.add(current, _frontier_from_heap(frontier), list(reached), infos, g, _h(current, goal), "TRACE CAY DOI KHANG. Minimax: agent da toi Goal, cong thuong lon.", iteration=expanded)
        ...
        for score, nb in scored:
            ...
            value_text = _game_trace_value_text(nb, stage, score, branch_label="branch_value", enemies=current_enemies)

...
def expectimax(stage: Stage) -> SearchResult:
    ...
    recorder = TraceRecorder("Expectimax", limit=MAX_TRACE_STEPS)
    ...
    while frontier and expanded < 300 and time.perf_counter() - start_time <= MAX_STAGE5_SECONDS:
        ...
        if current == goal:
            ...
            if recorder.can_record():
                recorder.add(current, _frontier_from_heap(frontier), list(reached), infos, g, _h(current, goal), "TRACE CAY DOI KHANG. Expectimax: agent da toi Goal, cong thuong lon.", iteration=expanded)
        ...
        for score, nb in scored:
            ...
            value_text = _game_trace_value_text(
                ...
                mode="expectimax",
                enemies=current_enemies,
                extra=f"nodes={int(context.get('nodes', 0))}",
            )

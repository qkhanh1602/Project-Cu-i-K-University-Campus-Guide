...
def csp_domain_preview_colors_from_note(note: str) -> Dict[GridPos, str]:
    """Return the first remaining domain color for each CSP building.

    Forward Checking prunes domains before a future building is actually
    assigned. Showing the first remaining value keeps the map aligned with the
    visible color-order queue without pretending those buildings are final.
    """
    out: Dict[GridPos, str] = {}
    for raw in str(note).splitlines():
        line = raw.strip()
        for key in BUILDING_ORDER:
            prefix = f"D({_building_label(key)})"
            if not line.startswith(prefix) or "=" not in line:
                continue
            domain_text = line.split("=", 1)[1]
            for code, label, color_hex in COLORS:
                if code in domain_text or label in domain_text:
                    out[BUILDING_DOORS[key]] = color_hex
                    break
    return out

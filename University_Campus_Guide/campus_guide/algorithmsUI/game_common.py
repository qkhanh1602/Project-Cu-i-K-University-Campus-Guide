...
def fixed_opponent_starts(stage: Stage, limit: int = 1) -> List[GridPos]:
    if stage.idx == 5:
...
def _initial_opponents(agent_pos: GridPos, stage: Stage, limit: int = 1) -> List[GridPos]:
...
def moving_opponent_positions(agent_pos: GridPos, stage: Stage, limit: int = 1) -> List[GridPos]:
...
def opponent_positions_for_route(route: List[GridPos] | Tuple[GridPos, ...], stage: Stage, limit: int = 1) -> List[GridPos]:
...
def _game_trace_value_text(
    pos: GridPos,
    stage: Stage,
    branch_value: float,
    *,
    branch_label: str = "branch_value",
    mode: str = "minimax",
    enemies: List[GridPos] | None = None,
    extra: str = "",
) -> str:
    state_score = _game_static_value(pos, stage, mode, enemies)
    parts = [
        f"state_score={state_score:.1f}",
        f"{branch_label}={branch_value:.1f}",
        f"state_breakdown: {_score_summary(pos, stage, enemies)}",
    ]
    if extra:
        parts.append(extra)
    return "; ".join(parts)

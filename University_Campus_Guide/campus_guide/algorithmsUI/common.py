...
def finish_search(
    ...
    # Stage context is used only by rich trace views.
    result.stage = stage
    return result
...
def frontier_from_heap(heap: List[Tuple], limit: int = 16) -> List[GridPos]:
    ...
    Some heap rows carry extra values such as cost or metadata, so this helper
    searches each row for the real GridPos and ignores non-position values.
    """

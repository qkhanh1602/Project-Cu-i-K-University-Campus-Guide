...
def alpha_beta(stage: Stage) -> SearchResult:
    ...
    recorder = TraceRecorder("Alpha-Beta Pruning", limit=700)
    ...
    while frontier and expanded < 700:
        ...
        if current == goal:
            ...
            recorder.add(current, _frontier_from_heap(frontier), list(reached), infos, g, _h(current, goal), "TRACE CAY DOI KHANG. Alpha-Beta: agent da toi Goal, cong thuong lon.", iteration=expanded)
        ...
        for score, nb in scored:
            ...
            if nb in reached:
                value_text = _game_trace_value_text(
                    nb,
                    stage,
                    score,
                    branch_label="branch_value",
                    enemies=current_enemies,
                    extra=f"alpha={_fmt_bound(alpha_trace)}; beta={_fmt_bound(beta_trace)}",
                )

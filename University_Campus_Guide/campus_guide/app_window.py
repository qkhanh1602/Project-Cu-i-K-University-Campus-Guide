...
from algorithmsUI.search_algorithms import SearchResult, TraceStep, path_cost, run_algorithm
from algorithmsUI.csp_common import (
    _shortest_segment,
    csp_color_assignments_from_note,
    csp_color_for_value_text,
    csp_domain_preview_colors_from_note,
)
...
def contiguous_path(cells: List[GridPos]) -> List[GridPos]:
    if len(cells) < 2:
        return cells[:]
    out = [cells[0]]
    for cell in cells[1:]:
        if manhattan(out[-1], cell) != 1:
            break
        out.append(cell)
    return out
...
class MainWindow(QMainWindow):
    ...
    def result_is_stopped(self) -> bool:
        if not self.result:
            return False
        status = str(self.result.status).lower()
        stopped_tokens = (
            "d\u1eebng",
            "dung",
            "d?ng",
        ...
        return any(token in status for token in stopped_tokens)

    def stopped_result_message(self) -> str:
        if not self.result:
            return "Thu\u1eadt to\u00e1n \u0111\u00e3 d\u1eebng."
        current = None
        if self.result.trace:
            idx = max(0, min(self.trace_index, len(self.result.trace) - 1))
            current = self.result.trace[idx].current
        elif self.result.path:
            current = self.result.path[-1]
        suffix = (
            f" Nh\u00e2n v\u1eadt \u0111\u1ee9ng t\u1ea1i {current}; kh\u00f4ng v\u1ebd \u0111\u01b0\u1eddng gi\u1ea3 t\u1edbi Goal."
            if current
            else " Kh\u00f4ng v\u1ebd \u0111\u01b0\u1eddng gi\u1ea3 t\u1edbi Goal."
        )
        return f"\u0110\u00e3 d\u1eebng: {self.result.status}.{suffix}"
    ...
    # Search execution and trace synchronization
    ...
    def ensure_result(self) -> None:
        if self.result is None:
            ...
            if self.result_is_stopped():
                self.status_label.setText(self.stopped_result_message())
            elif ok:
    ...
    def render_trace(
        ...
        is_game_map = self.stage.idx == 5 and is_game_trace(self.result.algorithm, step.mode)
        is_ida_map = self.result.algorithm == "IDA*"
        is_local_beam_map = self.result.algorithm == "Local Beam Search"
        map_current = step.current
        map_frontier = step.frontier
        map_reached = step.reached
        ...
        if is_ida_map:
            dfs_stack = contiguous_path(list(step.reached))
            if len(dfs_stack) >= 2:
                final_path = dfs_stack
        belief_branch_paths: List[List[GridPos]] = []
        belief_paths_data = getattr(self.result, "belief_paths", []) if is_belief_map else []
        belief_has_solution = bool(belief_paths_data) and self.result.status == "Ho\u00e0n th\u00e0nh" and index >= len(self.result.trace) - 1
        if belief_has_solution:
            belief_cells = []
            for path in belief_paths_data:
                if path:
                    path_index = min(index, len(path) - 1)
                    belief_cells.append(path[path_index])
                    belief_branch_paths.append(list(path[: path_index + 1]))
        ...
        if is_game_map and self.result.path:
            path_index = min(index, len(self.result.path) - 1)
            route_prefix = self.result.path[: path_index + 1]
            map_current = route_prefix[-1]
            map_frontier = []
            map_reached = route_prefix
            route_for_opponent = route_prefix
            final_path = self.result.path if keep_final_path or index >= len(self.result.trace) - 1 else route_prefix
        csp_try_cells: List[GridPos] = []
        csp_removed_cells: List[GridPos] = []
        csp_failed_cells: List[GridPos] = []
        ida_cut_cells: List[GridPos] = []
        ida_allowed_cells: List[GridPos] = []
        ida_skip_cells: List[GridPos] = []
        beam_chosen_cells: List[GridPos] = []
        beam_discarded_cells: List[GridPos] = []
        beam_paths: List[List[GridPos]] = []
        if is_stage6_csp:
            for n in step.neighbors:
                ...
                elif status in {"FAIL", "PRUNE", "CONFLICT"}:
                    csp_failed_cells.append(n.node)
        elif is_ida_map:
            for n in step.neighbors:
                status = str(n.status).upper()
                if status in {"ADD", "ADD/UPDATE", "ENQUEUE", "PUSH"}:
                    ida_allowed_cells.append(n.node)
                elif status in {"CUT", "CUTOFF"}:
                    ida_cut_cells.append(n.node)
                elif status in {"SKIP", "REJECT"}:
                    ida_skip_cells.append(n.node)
        elif is_local_beam_map:
            for n in step.neighbors:
                status = str(n.status).upper()
                if status in {"CHOSEN", "SELECTED", "KEEP"}:
                    beam_chosen_cells.append(n.node)
                elif status in {"CANDIDATE", "SKIP", "REJECT"}:
                    beam_discarded_cells.append(n.node)
            beam_paths = [contiguous_path(list(path)) for path in getattr(step, "beam_paths", [])]
            beam_paths = [path for path in beam_paths if len(path) >= 2]
        is_forward_checking_map = is_stage6_csp and "forward checking" in f"{self.result.algorithm} {step.mode}".lower()
        csp_color_cells = csp_color_assignments_from_note(step.note) if is_stage6_csp else None
        if is_forward_checking_map:
            domain_preview = csp_domain_preview_colors_from_note(step.note)
            affected_domain_cells = {
                n.node
                for n in step.neighbors
                if str(n.status).upper() == "REMOVE"
            }
            domain_preview = {
                pos: color
                for pos, color in domain_preview.items()
                if pos in affected_domain_cells
            }
            if csp_color_cells is None:
                csp_color_cells = domain_preview
            else:
                domain_preview.update(csp_color_cells)
                csp_color_cells = domain_preview
        if is_stage6_csp and csp_color_cells is not None:
            for n in step.neighbors:
                status = str(n.status).upper()
                if status == "REMOVE" and is_forward_checking_map:
                    continue
        ...
        self.map_view.set_trace_overlay(
            None if is_belief_map else map_current,
            map_frontier,
            map_reached,
            [] if is_belief_map or is_game_map else csp_try_cells if is_stage6_csp else ida_allowed_cells if is_ida_map else beam_chosen_cells if is_local_beam_map else [
                n.node
                for n in step.neighbors
            ...
            is_stage6_csp,
            csp_removed_cells if is_stage6_csp else ida_skip_cells if is_ida_map else beam_discarded_cells if is_local_beam_map else [],
            csp_failed_cells if is_stage6_csp else ida_cut_cells if is_ida_map else [],
            None,
            is_and_or_map,
            and_or_action,
            and_or_branch_paths,
            csp_color_cells,
            belief_branch_paths if is_belief_map else beam_paths if is_local_beam_map else [],
        )
    ...
    # Stage 6 and AND-OR route animation
    ...
    def next_and_or_route_cell(self) -> None:
        ...
        visible_branch_paths: List[List[GridPos]] = []
        for done_branch in self.and_or_route_branches[: self.and_or_route_branch_index]:
            if len(done_branch) > 1:
                visible_branch_paths.append(done_branch)
        if len(path_prefix) > 1:
            visible_branch_paths.append(path_prefix)

        result_states: List[GridPos] = []
        for candidate in visible_branch_paths:
            if len(candidate) > 1:
    ...
    def toggle_search_timer(self) -> None:
        ...
        if self.search_timer.isActive():
            ...
            if self.stage.idx == 5 and self.result and self.result.trace and self.trace_index >= len(self.result.trace) - 1:
                self.trace_index = 0
                self.render_trace(0)
            self.search_timer.start(self.speed_ms)
    ...
    def open_trace_dialog(self) -> None:
        ...
        self.trace_dialog.focus_tree_tab_if_needed()
